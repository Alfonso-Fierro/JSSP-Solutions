...
def schedule_visual(data_frame, num_m):
    ...
    colors = plt.cm.Set3(np.linspace(0, 1, num_colors)) # type: ignore
...
def precedence(order, matrix):
    FITS = True

    for action in order: 
        action_index = order.index(action)
        sequent = order[:action_index]
        precedent = order[action_index+1:]
        for i in sequent:
            # i must precede or not be comparable to the action
            if matrix[action][i] == 1:
                FITS = False
        for i in precedent:
            # the action must precede or not be comparable to i
            if matrix[i][action] == 1:
                FITS = False
    return FITS